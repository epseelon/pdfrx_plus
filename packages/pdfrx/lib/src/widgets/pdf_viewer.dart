...
class _PdfViewerState extends State<PdfViewer>
  ...
  /// Convenience accessor for the annotation controller owned by the
  /// associated [PdfViewerController]. Returns `null` until a controller
  /// is attached (i.e. until the document has finished loading).
  PdfAnnotationController? get _annotationController => _controller?._annotationController;
                        ...
                        ExcludeSemantics(
                          ...
                          child: iv.InteractiveViewer(
                            ...
                            boundaryProvider: widget.params.pageTransition == PageTransition.discrete
                                ...
                                : null,
                            maxScale: widget.params.maxScale ?? 8.0,
                            minScale: minScale,
    ...
    if (!_initialized && _layout != null && _fitScale != _defaultMinScale) {
      ...
      if (_currentZoom < _fitScale || _currentZoom == oldFitScale) {
        ...
        if (newPageSize > 0) {
          ...
          zoomTo = calculatedZoom.clamp(minScale, widget.params.maxScale ?? 8.0);
        } else {
          zoomTo = _currentZoom;
        }
      } else {
  ...
  void _calcFitScale([int? pageNumber]) {
    ...
    if (widget.params.useAlternativeFitScaleAsMinScale == true) {
  ...
  void _calcZoomStopTable() {
    ...
    final effectiveMaxScale = widget.params.maxScale ?? 8.0;
    while (z < effectiveMaxScale) {
      _zoomStops.add(z);
      z *= 2;
    }
    if (!_areZoomsAlmostIdentical(z, effectiveMaxScale)) {
      _zoomStops.add(effectiveMaxScale);
    }

    // Add smaller zoom stops down to the minimum scale
    if (widget.params.minScale != null || widget.params.useAlternativeFitScaleAsMinScale != true) {
      z = _zoomStops.first;
        ...
        overlayWidgets.add(
          Positioned(
            ...
            child: PdfAnnotationLayer(
              ...
              selectedTextPadding: widget.params.selectedTextPadding,
  ...
  void _paintPagesCustom(
    ...
    for (var i = 0; i < _document!.pages.length; i++) {
      ...
      final previewScaleLimit = getPageRenderingScale(
        ...
        page,
        _controller!,
        widget.params.onePassRenderingScaleThreshold ?? 200 / 72,
      );
  ...
  void _onWheelDelta(PointerScrollEvent event) {
    ...
    try {
      ...
      if (HardwareKeyboard.instance.isControlPressed) {
        ...
        final newZoom = (_currentZoom * (pow(1.2, zoomFactor))).clamp(minScale, widget.params.maxScale ?? 8.0);
...
class PdfViewerController extends ValueListenable<Matrix4> {
  ...
  /// Owns ink annotation strokes and the drawing mode flag. Lifetime is
  /// tied to this controller so callers can read [annotationModeListenable]
  /// before any [PdfViewer] mounts.
  final PdfAnnotationController _annotationController = PdfAnnotationController();
  ...
  /// Go to the specified document position.
  ///
  /// [documentOffset] is the position in document coordinates.
  /// [zoom] is the target zoom (defaults to current zoom).
  ...
  ///
  /// Safe to read before any [PdfViewer] is mounted; defaults to `false`.
  /// Use this to hide overlays / FABs while the user is drawing.
  ValueListenable<bool> get annotationModeListenable => _annotationController.annotationModeListenable;
  ...
  Future<void> exitAnnotationMode() async {
    await _annotationController.exitMode(onAnnotationsChanged: __state?.widget.params.onAnnotationsChanged);
  }
  ...
  String exportAnnotationsAsJson() => _annotationController.exportJson();
